...
while($res = mysqli_fetch_array($que)){
	$task_info = $res["Info"];
	$taskbar=$taskbar."<div class='task_job' id='task_job_$task_num' onclick='task_getinfo($task_num)' onchange='task_change($task_num)'><textarea class='task_job_textarea' id='task_$task_num' style='width:90%; border:none; padding:2px; background-color:#f2f2f2; resize:none;' rows='3'>$task_info</textarea><button class='task_job_end_button' id='task_butt_$task_num' onclick='task_done($task_num)'>x</button></div>";
	$task_num--;
}
